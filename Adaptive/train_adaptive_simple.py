...
def train_epoch(model, train_loader, optimizer, criterion, device, epoch):
    ...
    for batch_idx, (target_imgs, input_imgs, filename) in enumerate(progress_bar):
        ...
        # Print detailed loss info every 100 batches
        if batch_idx % 100 == 0:
            print(f"\nBatch {batch_idx}: Loss components:")
            print(f"  Charbonnier: {char_loss.item():.4f}")
            print(f"  Edge: {edge_loss_val.item():.4f}")
            print(f"  FFT: {fft_loss_val.item():.4f}")
            print(f"  SSIM: {ssim_loss_val.item():.4f}")
            print(f"  Illumination: {illum_loss_val.item():.4f}")
            print(f"  Adaptive: {adapt_loss.item():.4f}")
...
def main():
    parser = argparse.ArgumentParser(description='Train Adaptive NeRD-Rain Model (Simple Version)')
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        # Validate
        val_loss, val_psnr = validate_epoch(model, val_loader, criterion, device, epoch)
