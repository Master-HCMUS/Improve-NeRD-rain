...
from mlp import INR
from Adaptive.adaptive_inr import AdaptiveINR
...
class BFPU(nn.Module):
    def __init__(self, dim=48):
        super(BFPU, self).__init__()
        self.EB = BasicConv(dim, dim, kernel_size=3, relu=True, stride=1)
...
class AdaptiveMultiscaleNet(nn.Module):
    """
    Enhanced MultiscaleNet with Adaptive Query Resolution
    """
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
        ...
        super(AdaptiveMultiscaleNet, self).__init__()
        
        self.use_adaptive_inr = use_adaptive_inr
        ...
        # Adaptive or regular INR for small scale
        if self.use_adaptive_inr:
            self.INR = AdaptiveINR(dim, contrast_method=contrast_method, 
                                   min_density=min_density, max_density=max_density).cuda()
        else:
            self.INR = INR(dim).cuda()

        # Upsampling from small to mid
        self.upsmall2mid1 = Upsample(int(dim * 2 ** 2))
        self.upsmall2mid2 = BasicConv(int(dim * 2 ** 1), dim, kernel_size=3, relu=True, stride=1)

        # Mid scale components
        ...
        # Adaptive or regular INR for mid scale
        if self.use_adaptive_inr:
            self.INR2 = AdaptiveINR(dim, contrast_method=contrast_method,
                                    min_density=min_density, max_density=max_density).cuda()
        else:
            self.INR2 = INR(dim).cuda()

        # Upsampling from mid to max
        self.upmid2max1 = Upsample(int(dim * 2 ** 2))
        self.upmid2max2 = BasicConv(int(dim * 2 ** 1), dim, kernel_size=3, relu=True, stride=1)

        # Max scale components (similar structure as original model)
        ...
        # BFPU modules (unchanged)
        self.BFF1 = BFPU(dim * 4)
        self.BFF2 = BFPU(dim * 4)
        self.BFF3 = BFPU(dim * 4)

    def forward(self, inp_img):
        outputs = []
        
        # Multi-scale input preparation
        inp_img_max = inp_img
        ...
        # Apply Adaptive INR
        if self.use_adaptive_inr:
            INR = self.INR(latent_small_mid, inp_img_small)  # Pass input for contrast computation
        else:
        ...
        # Prepare features for second INR
        ...
        # Apply second Adaptive INR
        if self.use_adaptive_inr:
            INR2 = self.INR2(latent_mid_INR_max, mid_img)  # Pass input for contrast computation
        else:
        ...
        # (Following the original model structure for the remaining decoder parts)

        # BFPU operations
        BFF1 = self.BFF1(latent_max)
        latent_max = latent_max + BFF1
...
MultiscaleNet = AdaptiveMultiscaleNet