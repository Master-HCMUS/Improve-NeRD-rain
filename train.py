...
criterion_ssim = losses.SSIMLoss()
criterion_illum = losses.IlluminationAwareLoss()
...
for epoch in range(start_epoch, num_epochs + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        loss_ssim = criterion_ssim(restored[0], target[0]) + criterion_ssim(restored[1], target[1]) + criterion_ssim(restored[2], target[2])
        loss_illum = criterion_illum(restored[0], target[0]) + criterion_illum(restored[1], target[1]) + criterion_illum(restored[2], target[2])
        
        loss = loss_char + 0.01 * loss_fft + 0.05 * loss_edge + 0.1 * loss_l1 + 0.2 * loss_ssim + 0.1 * loss_illum
        loss.backward()
        optimizer.step()
        ...
        writer.add_scalar('loss/ssim_loss', loss_ssim, iter)
        writer.add_scalar('loss/illum_loss', loss_illum, iter)
